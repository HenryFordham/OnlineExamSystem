...
function FirstPageUp() {
    ...
    var no=parseInt(sessionStorage.getItem("Page"));
	if (no==3){
		alert("这已经到了最后一页了，请注意");
	}
    if (no==4) {
    	alert("这已经是最后一题，如果你想提交答案，请点击提交");
    	return;
    }
    var newPage=parseInt(sessionStorage.getItem("Page"))+1;
    sessionStorage.setItem("Page",newPage.toString());
    return;
}

function FirstPageDown() {
    if (sessionStorage.getItem("Page")==null||parseInt(sessionStorage.getItem("Page"))==1){
    	alert("这是第一页，没有上一页了");
        return;
    }
    var newPage=parseInt(sessionStorage.getItem("Page"))-1;
